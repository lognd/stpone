...
Desktop core ported: typani replaces the homebrew Result family, logging follows the frob scaffold layout, and the serial packet types are carried over with the five comp-0701 stream bugs fixed so composite packets can be declared and streamed (UT-0601, UT-0602, UT-0701..0706).
...
```
 ...
 .../SUB-04-hardware-harnesses.md                   |  24 ++
 ...
 91 files changed, 5564 insertions(+), 7 deletions(-)
```

### Evidence
- `tests/integration/test_wiring.py::test_package_exports_typani_vocabulary` (pytest node id, verified passing when recorded)
- `tests/unit/test_logging.py::test_error_ctx_logs_and_reraises` (pytest node id, verified passing when recorded)
- `tests/integration/test_serial_stream.py::test_handshake_round_trip` (pytest node id, verified passing when recorded)
- `tests/integration/test_serial_stream.py::test_composite_packet_round_trip` (pytest node id, verified passing when recorded)

### Captured claims
- tests: 4 passed (from 4 evidence id(s))
- gates: 5 error(s), 31 warning(s), 44 waived
- error-findings: MILE003@tickets.md, PRE001@tickets/T-0002, REF002@docs/index.md, REF002@tests/avr/hello_world.cpp, TEST006@.frob/coverage-stamp
