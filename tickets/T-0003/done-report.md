...
scripts/flash.py replaces setup_usb.bat, flash.bat and upload.sh with one pydantic-configured, typani-Result state machine: cmake build, WSL detection, usbipd attach by hardware id (elevated bind through PowerShell once), port wait, and the legacy avrdude invocation; plain Linux skips the forwarding step (UT-0501..0509, CT-020, CT-021).
...
```
 ...
 94 files changed, 5691 insertions(+), 14 deletions(-)
```

### Evidence
- `tests/unit/test_flash_app.py::test_flash_on_wsl_attaches_first` (pytest node id, verified passing when recorded)
- `tests/unit/test_flash_app.py::test_flash_on_linux_skips_attach` (pytest node id, verified passing when recorded)
- `tests/unit/test_flash_host.py::test_bind_elevates_through_powershell` (pytest node id, verified passing when recorded)
- `tests/integration/test_flash_cli.py::test_all_target_requires_build_only` (pytest node id, verified passing when recorded)

### Captured claims
- tests: 4 passed (from 4 evidence id(s))
- gates: 1 error(s), 27 warning(s), 44 waived
